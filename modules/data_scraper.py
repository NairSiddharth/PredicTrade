...
class DataScraper:
    ...
    def calculate_sentiment_fear_greed_index(self, stock_tickers: List[str]) -> pd.DataFrame:
        ...
        try:
            ...
            trends_data = self.get_optimized_trends_data(stock_tickers)

            self.logger.info("Fetching news sentiment...")
            news_data = self.get_news_sentiment(stock_tickers)

            self.logger.info("Fetching Finnhub sentiment...")
            finnhub_data = self.get_finnhub_sentiment(stock_tickers)

            self.logger.info("Fetching market fear context...")
            market_fear = self.get_market_fear_context()
